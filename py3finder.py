...
def parse_pypi():
    ...
    pypi = "https://pypi.python.org/pypi?:action=browse&show=all&c=533"
    page = requests.get(pypi)
...
def parse_portingdb():
    """Picks gray and red packages from PortingDB and
    returns them as a list.
    """

    string = get_html("http://fedora.portingdb.xyz/")

    idle_re = r'DDDDDD">&nbsp;<\/span>&nbsp;<a href="\/pkg\/([\w]*-?[\w]*)\/">'
    blck_re = r'D9534F">&nbsp;<\/span>&nbsp;<a href="\/pkg\/([\w]*-?[\w]*)\/">'
    idle_packages = re.findall(idle_re, string)
    blocked_pacakages = re.findall(blck_re, string)
